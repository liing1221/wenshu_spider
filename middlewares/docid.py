...
"""
    ...
    @File        : docid.py
    @Create_Date : 2019-05-15 12:55
    @Update_Date : 2019-05-15 12:55
    @Author      : liing
...
from middlewares.js import js_doc
...
def decode_docid(runeval, docid):
    """
    解密docid,返回解密后的字符串
    :param runeval: RunEval 为检索响应列表第一项
    :param docid: docid 为检索响应列表的文书ID
    :return:
    """
    key = getkey(runeval).encode('utf-8')
    new_docid = Decrypt(unzip(docid), key).replace('\f', '')
    return new_docid
...
if __name__ == '__main__':
    ...
    value = '532bd8ed-4ba8-48b7-ad70-0063f64ede05'
    new_docid = decode_docid(RunEval, docid)
    print(bool(value == new_docid))
    print('ok')