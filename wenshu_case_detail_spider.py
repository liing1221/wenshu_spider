...
from middlewares.settings import RETRY, TIMEOUT, CONTRAST_TABLE, LEN_SQR, DATA_FILE, PROCESSES
    ...
    def parse_page(self, response):
        ...
        try:
            ...
            if "PJJG" in names and 'WBWB' in names:
                ...
                pjjg = re.search(r'(<a type=\'dir\' name=\'PJJG\'.*?)<div style=\'TEXT-ALIGN: right;',
                                 response.text, re.S)
                if pjjg:
                    item['PJJG'] = pjjg.group(1)
        ...
        item['SPZ'] = item['SPY'] = item['DLSPY'] = item['SJY'] = item['PSY'] = key = ''
        for wbwb in wbwbs:
            if len(wbwb) > 20:
                break
